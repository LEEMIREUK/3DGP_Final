...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_SPACE:	
			//shooting
			if (m_pScene->m_iBullet < m_pScene->m_iBulletNum)
			{
				m_pScene->BuildBullet(*m_pPlayer, m_pScene->m_iBullet);
				m_pScene->m_iBullet++;
			}
			else
				m_pScene->m_iBullet = 0;
			break;
		case 0x52:	// 'R'
			ReleaseObjects();
			BuildObjects();
...
void CGameFramework::ProcessInput()
{
	...
	if (::GetKeyboardState(pKeysBuffer))
	{
		...
		// C
		if (pKeysBuffer[0x43] & 0xF0) dwDirection |= DIR_UP;
		// Z
		if (pKeysBuffer[0x5A] & 0xF0) dwDirection |= DIR_DOWN;
	}
...
void CGameFramework::FrameAdvance()
{
	...
	float pfClearColor[4] = { 0.1f, 0.2f, 0.1f, 1.f };
