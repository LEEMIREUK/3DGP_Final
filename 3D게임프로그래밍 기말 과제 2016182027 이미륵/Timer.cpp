...
void CGameTimer::Tick(float fLockFPS)
{
	...
	fTimeOver = float((m_nCurrentPerformanceCounter - m_nLastPerformanceCounter) * m_fTimeScale);

	if (fLockFPS > 0.0f)
	{
		while (fTimeOver < (1.0f / fLockFPS))
		{
			::QueryPerformanceCounter((LARGE_INTEGER*)&m_nCurrentPerformanceCounter);
			fTimeOver = float((m_nCurrentPerformanceCounter - m_nLastPerformanceCounter) * m_fTimeScale);
		}
	...
	if (fabsf(fTimeOver - m_fTimeOver) < 1.0f)
	{
		::memmove(&m_fFrameTime[1], m_fFrameTime, (MAX_SAMPLE_COUNT - 1) * sizeof(float));
		m_fFrameTime[0] = fTimeOver;
		if (m_nSampleCount < MAX_SAMPLE_COUNT) m_nSampleCount++;
	}

	m_nFramesPerSecond++;
	m_fFPSTimeElapsed += fTimeOver;
	...
	m_fTimeOver = 0.0f;
	for (ULONG i = 0; i < m_nSampleCount; i++) m_fTimeOver += m_fFrameTime[i];
	if (m_nSampleCount > 0) m_fTimeOver /= m_nSampleCount;
}
