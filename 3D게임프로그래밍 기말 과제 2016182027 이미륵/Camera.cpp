...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, (XMFLOAT3&)m_pPlayer->GetPosition());
...
{
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, (XMFLOAT3&)m_pPlayer->GetUpVector());
